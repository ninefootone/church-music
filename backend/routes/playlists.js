...
router.post('/', requireAuth, requirePermission('can_manage_playlists'), async function(req, res, next) {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO church_playlists (church_id, name, url, sort_order)
       VALUES ($1, $2, $3, (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM church_playlists WHERE church_id = $1))
       RETURNING *`,
      [req.churchId, name, url]
    );
...
router.put('/:id', requireAuth, requirePermission('can_manage_playlists'), async function(req, res, next) {
  ...
  try {
    const result = await pool.query(
      `UPDATE church_playlists SET name = $1, url = $2
       WHERE id = $3 AND church_id = $4
       RETURNING *`,
      [name, url, req.params.id, req.churchId]
    );
