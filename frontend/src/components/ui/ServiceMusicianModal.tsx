...
interface Props {
  serviceId: string
  onAdd: (musician: { id: string; name: string; role: string; user_id: string | null }) => void
  onClose: () => void
}
...
export function ServiceMusicianModal({ serviceId, onAdd, onClose }: Props) {
  ...
  const finalName = selectedMember ? selectedMember.name : guestName.trim()
  const finalRole = role === '__custom__' ? customRole.trim() : role

  const canSubmit = finalName.length > 0 && finalRole.length > 0
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const res = await api.post(`/api/services/${serviceId}/musicians`, {
        name: finalName,
        role: finalRole,
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
      ...
      <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0', width: '100%', maxWidth: 520, padding: 'var(--space-lg)', paddingBottom: 'calc(var(--space-lg) + env(safe-area-inset-bottom))' }}>
          ...
          {selectedMember ? (
            ...
            <div>
              <input
                ...
                style={{ width: '100%', padding: '10px 14px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-md)', background: 'var(--color-surface)', color: 'var(--color-text-primary)', boxSizing: 'border-box' }}
                autoFocus
              />
              {filtered.length > 0 && (
                <div style={{ border: '1px solid var(--color-border)', borderTop: 'none', borderRadius: '0 0 var(--radius-sm) var(--radius-sm)', overflow: 'hidden' }}>
                  {filtered.map(m => (
                    <button
                      ...
                      style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 14px', background: 'var(--color-surface)', border: 'none', cursor: 'pointer', fontSize: 'var(--text-md)', borderBottom: '1px solid var(--color-border)' }}
                    >
              ...
              {query.length > 0 && filtered.length === 0 && (
                <div style={{ padding: '8px 14px', fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', borderTop: 'none', borderRadius: '0 0 var(--radius-sm) var(--radius-sm)' }}>
                  No members found — will be added as a guest
                </div>
              )}
              {isGuest && query.length > 0 && (
                <input
                  type="hidden"
                  onChange={() => setGuestName(query)}
                />
              )}
            </div>
          )}

          {/* Guest name clarification */}
          {!selectedMember && query.length > 0 && filtered.length === 0 && (
            <div style={{ marginTop: 8 }}>
              <input
                type="text"
                placeholder="Confirm guest name"
                value={guestName || query}
        ...
        <div style={{ marginBottom: 'var(--space-lg)' }}>
          <label style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 6 }}>Role / instrument</label>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: role === '__custom__' ? 10 : 0 }}>
            {PRESET_ROLES.map(r => (
              <button
                key={r}
                ...
                style={{
                  ...
                  border: '1px solid',
                  borderColor: role === r ? 'var(--color-brand-600)' : 'var(--color-border)',
                  background: role === r ? 'var(--color-brand-600)' : 'var(--color-surface)',
            ...
            <button
              onClick={() => setRole(role === '__custom__' ? '' : '__custom__')}
              style={{
                padding: '6px 14px',
                borderRadius: 'var(--radius-full)',
                border: '1px solid',
                borderColor: role === '__custom__' ? 'var(--color-brand-600)' : 'var(--color-border)',
                background: role === '__custom__' ? 'var(--color-brand-600)' : 'var(--color-surface)',
                color: role === '__custom__' ? '#fff' : 'var(--color-text-secondary)',
                fontSize: 'var(--text-sm)',
                fontWeight: 500,
                cursor: 'pointer',
              }}
