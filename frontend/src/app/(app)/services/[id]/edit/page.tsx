...
export default function ServiceEditPage() {
  ...
  const addSong = (song: Song) => {
    setItems(prev => [...prev, {
      ...
      song_default_key: song.default_key,
      song_category: song.category,
      title: '',
      notes: '',
      key_override: song.default_key || '',
      expanded: false,
    }])
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-lg)' }}>
        <div>
          ...
          <h1 className="page-title">
            ...
            {service.service_time && <span style={{ fontWeight: 400, color: 'var(--color-text-secondary)', fontSize: 'var(--text-xl)' }}> · {service.service_time}</span>}
          </h1>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 'var(--space-md)', alignItems: 'start' }}>
        ...
        <div>
          ...
          {items.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: 'var(--space-xl)', color: 'var(--color-text-muted)' }}>
              Add songs and items from the panel on the right
            </div>
          ) : (
            <div>
              {items.map((item, idx) => (
                ...
                >
                  <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px var(--space-md)' }}>
                      {/* Drag handle */}
                      <div style={{ cursor: 'grab', color: 'var(--color-text-muted)', flexShrink: 0, display: 'flex', alignItems: 'center' }}>
                    ...
                    {item.expanded && (
                      <div style={{ padding: '0 var(--space-md) var(--space-sm)', paddingLeft: 64, borderTop: '1px solid var(--color-border)' }}>
                        <input
                          ...
                          placeholder="Notes for this item (e.g. Capo 2, use acoustic intro…)"
        ...
        {/* Right — add songs / items */}
        <div style={{ position: 'sticky', top: 80 }}>

          {/* Add song */}
          <div className="card" style={{ marginBottom: 'var(--space-md)' }}>
            <div className="section-label">Add a song</div>
            ...
            <div style={{ maxHeight: 320, overflowY: 'auto', margin: '0 calc(-1 * var(--space-lg))', padding: '0 var(--space-lg)' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--color-surface)', borderTop: '1px solid var(--color-border)', padding: '14px var(--space-lg)', display: 'flex', justifyContent: 'flex-end', gap: 8, zIndex: 50, boxShadow: '0 -2px 8px rgba(0,0,0,0.06)' }}>
        <Link href={`/services/${id}`} className="btn btn-secondary">Cancel</Link>
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
          {saving ? 'Saving…' : `Save running order (${items.length} items)`}
        </button>
      </div>
